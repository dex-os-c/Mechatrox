...
import { RobotModel, NetworkField, INTRO_START, REST_POSE } from '../three/Hero3D'
import { eventInfo } from '../data/events'

export default function Hero({ ready }) {
  const runwayRef = useRef(null)
  const textWrapRef = useRef(null)
  const [robotGroup, setRobotGroup] = useState(null)
  const introProgress = useRef(0)
  const startedRef = useRef(false)

  // Entrance text reveal (unrelated to scroll — this just plays once on load)
  useEffect(() => {
    if (!ready) return
  ...
  // The scroll-driven mech entrance: flies in, spins, scales up as you scroll down the
  // runway — and, because this is a GSAP scrub (tied directly to scroll position rather
  // than played once), reverses smoothly if you scroll back up. Idle mouse-parallax hands
  // off control at the very end (see the introProgress gate inside RobotModel's useFrame).
  useEffect(() => {
    if (!ready || !robotGroup || !runwayRef.current || startedRef.current) return
    ...
    if (reduceMotion) {
      gsap.set(g.position, { x: REST_POSE.position[0], y: REST_POSE.position[1], z: REST_POSE.position[2] })
      gsap.set(g.rotation, { x: REST_POSE.rotation[0], y: REST_POSE.rotation[1], z: REST_POSE.rotation[2] })
      gsap.set(g.scale, { x: REST_POSE.scale, y: REST_POSE.scale, z: REST_POSE.scale })
      introProgress.current = 1
      return undefined
    }

    gsap.set(g.position, { x: INTRO_START.position[0], y: INTRO_START.position[1], z: INTRO_START.position[2] })
    gsap.set(g.rotation, { x: INTRO_START.rotation[0], y: INTRO_START.rotation[1], z: INTRO_START.rotation[2] })
    gsap.set(g.scale, { x: INTRO_START.scale, y: INTRO_START.scale, z: INTRO_START.scale })

    const restY = REST_POSE.rotation[1]
    ...
    tl.to(g.position, {
      x: REST_POSE.position[0], y: REST_POSE.position[1], z: REST_POSE.position[2],
      ease: 'power2.inOut', duration: 1,
    }, 0)
      .to(g.rotation, { y: restY - Math.PI * 2 * 2, ease: 'power1.inOut', duration: 1 }, 0)
      .to(g.rotation, { x: 0, z: 0, ease: 'power2.out', duration: 0.6 }, 0.1)
      .to(g.scale, { x: REST_POSE.scale, y: REST_POSE.scale, z: REST_POSE.scale, ease: 'back.out(1.4)', duration: 0.9 }, 0.05)
      .to(textWrapRef.current, { opacity: 0, y: -50, ease: 'power1.in', duration: 0.4 }, 0)
  ...
  return (
    <div ref={runwayRef} className="relative h-[230vh]">
      <section className="sticky top-0 h-[100dvh] overflow-hidden flex flex-col justify-end pt-[78px]">
        <div className="absolute inset-0 z-0 opacity-95">
          <Canvas camera={{ position: [1.2, 1.3, 5.2], fov: 42 }} dpr={[1, 1.6]}>
            ...
            <Suspense fallback={null}>
              <NetworkField />
              <RobotModel onReady={setRobotGroup} introProgress={introProgress} />
            </Suspense>
          </Canvas>
        </div>
        <div
        ...
        <div ref={textWrapRef} className="relative z-[2] wrap w-full">
