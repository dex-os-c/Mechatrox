...
export default function Hero({ ready }) {
  ...
  // The actual scroll-driven mech entrance: flies in, spins, scales up, then hands off
  // to idle mouse-parallax (see introProgress gate inside RobotModel's useFrame).
  useEffect(() => {
    if (!ready || !robotGroup || !runwayRef.current || startedRef.current) return
    startedRef.current = true

    const g = robotGroup
    gsap.set(g.position, { x: 4.6, y: -3.2, z: -2.4 })
    gsap.set(g.rotation, { x: 0, y: -3.4, z: 0.4 })
    gsap.set(g.scale, { x: 0.12, y: 0.12, z: 0.12 })
    ...
    tl.to(g.position, { x: 1.15, y: -0.9, z: 0, ease: 'power2.inOut', duration: 1 }, 0)
      .to(g.rotation, { y: restY - Math.PI * 2 * 2, ease: 'power1.inOut', duration: 1 }, 0)
      .to(g.rotation, { x: 0, z: 0, ease: 'power2.out', duration: 0.6 }, 0.1)
      .to(g.scale, { x: 1, y: 1, z: 1, ease: 'back.out(1.4)', duration: 0.9 }, 0.05)
